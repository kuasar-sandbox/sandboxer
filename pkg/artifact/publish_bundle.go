...
func (p *Publisher) publishBundle(
	...
	exactRoot, exactDependencies, remoteDependencies, err := selectExactBundleSources(ctx, opened, root, dependencies)
	if err != nil {
		return PublishResult{}, err
	}
	...
	ref, err := target.PutBundle(ctx, bundlePublishPlan{
		...
		exactRoot: exactRoot, exactDependencies: exactDependencies, remoteDependencies: remoteDependencies,
...
func selectExactBundleSources(
...
) (manifestbundle.ExactManifest, []manifestbundle.ExactManifest, int, error) {
	if opened == nil || opened.ManifestFetcher() == nil {
		return manifestbundle.ExactManifest{}, nil, 0, errors.New("publish Bundle: source selector is unavailable")
	}
	rootSource, err := opened.ManifestFetcher().SelectRoot(root)
	if err != nil {
		return manifestbundle.ExactManifest{}, nil, 0, err
	}
	if rootSource.Reader == nil {
		return manifestbundle.ExactManifest{}, nil, 0, errors.New("publish Bundle: root is not backed by the current Bundle")
	}
	...
	for _, key := range dependencies {
		...
		if err != nil {
			return manifestbundle.ExactManifest{}, nil, 0, fmt.Errorf("publish Bundle dependency %s: %w", manifest.HexKey(key), err)
		}
		...
		if err != nil {
			return manifestbundle.ExactManifest{}, nil, 0, fmt.Errorf("publish Bundle remote dependency %s: %w", manifest.HexKey(key), err)
		}
		consumeErr := consumeLocationSource(ctx, stream)
		closeErr := stream.Close()
		if err := errors.Join(consumeErr, closeErr); err != nil {
			return manifestbundle.ExactManifest{}, nil, 0, fmt.Errorf("verify Bundle remote dependency %s: %w", manifest.HexKey(key), err)
		}
		remoteDependencies++
	}
	return exactRoot, exactDependencies, remoteDependencies, nil
}
