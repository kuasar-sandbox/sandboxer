...
`resources.capacity` 是 guest-visible VM capacity,进入 Portable config. `resources.allocatable` 是 cold start 的 workload 默认值,也进入 Portable config;其中 `allocatable.cpu` 必须是有限数,且满足 `0 < allocatable.cpu <= capacity.cpu`. Restore 保持 E 中的 capacity,但可以从目标节点显式重新应用 allocatable;该运行时 policy 不改写 E 或 C0. `control`、`overhead`、`watermark_high` 和 `startup` 是 node policy,不进入 E.
...
Memory S capture记录 CH config/state 和 memfd sparse content;资源 policy 不写入 `snapshot.cfg`. Restore capacity identity来自 E,host若显式给出必须一致. Allocatable、`startup`、`overhead`、`watermark_high` 和 resource controller binding来自当前 host;Snapshot 中的 `BudgetAtSnapshot` 仍是 restore initial Budget 的权威值.
...
Host-only允许项包括 network provider/current identity、cgroup/controller、allocatable/resource enforcement、kernel/runtime actual path、active diff/template、restore prefetch和timeouts. Immutable disk graph、capacity identity、network topology由E拥有.
