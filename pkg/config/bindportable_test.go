...
func TestBindPortableDiskGraph(t *testing.T) {
	...
	cfg := &SandboxConfig{Boot: BootConfig{
		Root: RootConfig{Base: "file://image.erofs@digest:" + strings.Repeat("a", 64), Overlay: &OverlayConfig{
			Base: "self", BaseFromRefs: []string{"manifest://" + strings.Repeat("1", 64)},
		}},
		Disks: []DiskConfig{{Name: "data", RootConfig: RootConfig{Base: "file://data.overlay@digest:" + strings.Repeat("b", 64)}}},
	}}
	self := "file://" + filepath.Join(dir, "current.sandbox") + "@digest:" + strings.Repeat("c", 64)
	...
	if want := "file://" + filepath.Join(dir, "image.erofs") + "@digest:" + strings.Repeat("a", 64); cfg.Boot.Root.Base != want {
		t.Fatalf("root base = %q, want %q", cfg.Boot.Root.Base, want)
	}
	if want := "file://" + filepath.Join(dir, "data.overlay") + "@digest:" + strings.Repeat("b", 64); cfg.Boot.Disks[0].Base != want {
...
func TestBindPortableDiskGraphRejectsMissingDirectoryAndSelf(t *testing.T) {
	cfg := &SandboxConfig{Boot: BootConfig{Root: RootConfig{Base: "self", BaseFromRefs: []string{"file://lower@digest:" + strings.Repeat("a", 64)}}}}
